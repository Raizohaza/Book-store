    ...
    class QueryForSQLServer
    {
        ...
        public static DataTable UpdateProduct(Product product)
        {
            string Query = "update Product " +
                "set CatId = "+ product.CatId + ",SKU = '"+ product.SKU + "',Name = '" + product.Name + "',Price = " + product.Price + ",Quantity=" + product.Quantity + ",Description='" + product.Description + "',Image='be1abedd-c60c-409b-ac31-38d62f696a68' " +
                "where Id = " + product.Id ;
            DataTable dt = null;
            Provider p = new Provider();
            var products = new ObservableCollection<Product>();
